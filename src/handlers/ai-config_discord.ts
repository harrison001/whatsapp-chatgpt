...
const aiConfig_discord: IAiConfig = {
	dalle: {
...
const handleMessageAIConfig_discord = async (message: Message, prompt: any) => {
	try {
		...
		if (args.length === 2 && args[0].toLowerCase() === "tts" && (args[1].toLowerCase() === "yes" || args[1].toLowerCase() === "no")) {
		    aiConfig_discord.ttsEnabled = args[1].toLowerCase() === "yes";
		...
		*/
		...
		aiConfig_discord[target][type] = value;
...
export { aiConfig_discord, handleMessageAIConfig_discord };
